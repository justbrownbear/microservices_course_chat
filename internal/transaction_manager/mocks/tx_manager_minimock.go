...
// TxManagerMock implements transaction_manager.TxManager
...
// NewTxManagerMock returns a mock for transaction_manager.TxManager
...
// WithTransaction implements transaction_manager.TxManager
	...
	if mmWithTransaction.WithTransactionMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmWithTransaction.t.Errorf("TxManagerMock.WithTransaction got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

			if mm_want_ptrs.handler != nil && !minimock.Equal(*mm_want_ptrs.handler, mm_got.handler) {
				mmWithTransaction.t.Errorf("TxManagerMock.WithTransaction got unexpected parameter handler, want: %#v, got: %#v%s\n", *mm_want_ptrs.handler, mm_got.handler, minimock.Diff(*mm_want_ptrs.handler, mm_got.handler))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmWithTransaction.t.Errorf("TxManagerMock.WithTransaction got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}
...
func (m *TxManagerMock) MinimockWithTransactionInspect() {
	for _, e := range m.WithTransactionMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to TxManagerMock.WithTransaction with params: %#v", *e.params)
		}
	}
	...
	if m.WithTransactionMock.defaultExpectation != nil && afterWithTransactionCounter < 1 {
		if m.WithTransactionMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to TxManagerMock.WithTransaction")
		} else {
			m.t.Errorf("Expected call to TxManagerMock.WithTransaction with params: %#v", *m.WithTransactionMock.defaultExpectation.params)
		}
	}
	...
	if !m.WithTransactionMock.invocationsDone() && afterWithTransactionCounter > 0 {
		m.t.Errorf("Expected %d calls to TxManagerMock.WithTransaction but found %d calls",
			mm_atomic.LoadUint64(&m.WithTransactionMock.expectedInvocations), afterWithTransactionCounter)
	}
}
